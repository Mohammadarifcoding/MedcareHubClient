...
const MedicineItem = ({ filter }) => {
    ...
    useEffect(() => {
        const rangeMedicine = medicine?.filter((item) => item.Price <= priceRange);
        // console.log(rangeMedicine);
        ...
        <>
            ...
            {!isLoading ? (
                <div className="container mx-auto grid grid-cols-1 gap-6  md:grid-cols-2 lg:grid-cols-3">
                    {filteredMedicine
                        ?.filter((medicine) => medicine.status === 'Accepted')
                        ?.map((medicine) => (
